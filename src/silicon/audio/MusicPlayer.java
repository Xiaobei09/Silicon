...
public class MusicPlayer {
    ...
    private static final Pattern EXT_WHITELIST = Pattern.compile("(?i)\\.(ogg|mp3|wav|flac|m4a|wma|aac|opus|alac|aiff|aif|ape|wv|ac3|dts|tta|tak|mp2|mp1|au|snd|oga|spx|weba|mka|m4b|m4r|3ga|mp4|mkv|webm|avi|mov|flv|wmv|mpg|mpeg|3gp|3g2|m2ts|ts|mts|vob|asf|rm|rmvb|f4v|m4v|m2v|divx|xvid|ogv|mxf|nut|yuv)$");
    /** Soloud 内置解码器可解码的格式（stb_vorbis=ogg / dr_mp3=mp3 / stb_wav=wav）。其余音频/视频需经 AudioTranscoder 转码为 ogg/wav 后播放 */
    ...
    private static boolean advanceSafely(int delta) {
        // 激活专辑时只在专辑内切换；否则全曲目库切换；自动跳过不可播曲目（修复“全部变 game2”）
        ...
        if (loopMode == LOOP_RANDOM) {
            // 真随机：仅在可播子集内抽签，避免抽到不可播曲目后静默失败
            Seq<Integer> playable = new Seq<>();
            if (scope == null) {
                for (int i = 0; i < tracks.size; i++) if (isPlayableIndex(i)) playable.add(i);
            } else {
                for (int idx : scope) if (isPlayableIndex(idx)) playable.add(idx);
            }
            if (playable.size == 0) return false;
            if (playable.size == 1) {
                nextPos = playable.get(0);
            } else {
                int r = arc.math.Mathf.random(playable.size - 1);
                int pick = playable.get(r);
            ...
            if (nextPos < 0 || nextPos >= tracks.size) nextPos = playable.get(0);
            ...
            for (int idx : order) if (isPlayableIndex(idx)) { anyPlayable = true; break; }
            ...
            int nxt;
            if (cur < 0) {
                nxt = delta > 0 ? 0 : order.length - 1;
            } else {
                nxt = ((cur + delta) % order.length + order.length) % order.length;
            }
            // 沿 delta 方向扫描直到命中可播
            while (attempts < order.length) {
                int cand = order[nxt];
                if (isPlayableIndex(cand)) { nextPos = cand; break; }
                nxt = ((nxt + delta) % order.length + order.length) % order.length;
                attempts++;
            }
            if (nextPos < 0) return false;
        } else {
            // 列表循环等：沿 delta 方向线性扫描，跳过不可播
            boolean anyPlayable = false;
            if (scope == null) {
                for (int i = 0; i < tracks.size; i++) if (isPlayableIndex(i)) { anyPlayable = true; break; }
            } else {
                for (int idx : scope) if (isPlayableIndex(idx)) { anyPlayable = true; break; }
            }
            if (!anyPlayable) return false;
            int nxt;
            if (pos < 0) {
                nxt = delta > 0 ? 0 : size - 1;
            } else {
                nxt = ((pos + delta) % size + size) % size;
            }
            int attempts = 0;
            while (attempts < size) {
                int global = (scope == null) ? nxt : scope[nxt];
                if (isPlayableIndex(global)) { nextPos = global; break; }
                nxt = ((nxt + delta) % size + size) % size;
                attempts++;
            }
            if (nextPos < 0) return false;
        }
    ...
    private static void beginPlayback(int index) {
        ...
        // Soloud 仅原生支持 ogg/mp3/wav，其余音频/视频（flac/m4a/.../mp4/mkv/webm 等）需经 AudioTranscoder 转码为 ogg/wav 后播放；
        // 转码仅影响本机播放，不影响字节共享（他人仍可按原始字节解码或自行转码）
        if (t.isUrl() || t.isLocal()) {
            Fi decodable = AudioTranscoder.ensureDecodable(file, t.cacheHash);
    ...
    public static float trackLengthOf(MusicTrack t) {
        ...
        if (t.isUrl()) {
            ...
            float l = f == null ? -1f : readLengthFrom(f);
            if (l <= 0f) {
                Fi tr = AudioTranscoder.getTranscodedIfExists(t.cacheHash);
                if (tr != null) l = readLengthFrom(tr);
            }
            return l;
        }
        ...
        if (len <= 0f) {
            Fi tr = AudioTranscoder.getTranscodedIfExists(t.cacheHash);
            if (tr != null) len = readLengthFrom(tr);
        }
        return len;
    }
    ...
    /** 指定曲目是否可在本机直接播放（用于上下一切换时自动跳过不可播曲目，避免全部变 game2）。
     *  对 Soloud 原生不支持但可转码的格式（flac/m4a/.../mp4/mkv 等）也视为可播——播放时由 AudioTranscoder 转码为 ogg/wav */
    private static boolean isPlayable(MusicTrack t) {
        if (t == null) return false;
        if (t.isInternal()) return true;
        // 已有本地缓存 → 只要是音频/视频即视为可播（原生可解或可转码）
        try {
            Fi cached = cacheFileOf(t);
            if (cached != null && cached.exists() && cached.length() > 0) {
                String p = cached.absolutePath();
                if (isDecodablePath(p)) return true;
                String sniff = sniffFileExt(cached);
                if (sniff != null && isDecodablePath("a" + sniff)) return true;
                if (AudioTranscoder.isTranscodableExtension(p)) return true;
                if (sniff != null && AudioTranscoder.isTranscodableExtension("a" + sniff)) return true;
                // 缓存存在但为未知非媒体（如 .txt）→ 不可播
                return false;
            }
        } catch (Exception ignored) {}
        if (t.isUrl()) {
            // URL 未缓存：只要是 http(s) 且扩展在广义白名单内（或无扩展）即视为可播（下载后嗅探/转码）
            if (!(t.source.startsWith("http://") || t.source.startsWith("https://"))) return false;
            boolean hasMediaExt = false;
            try { hasMediaExt = EXT_WHITELIST.matcher(t.source).find(); } catch (Exception ignored) {}
            if (hasMediaExt) return true;
            // 无扩展名的 http 链接（如直播流）也放行，由下载后嗅探/转码决定
            return true;
        }
        if (t.isLocal()) {
            Fi orig = originalLocalFile(t);
            if (orig == null || !orig.exists() || orig.isDirectory() || orig.length() == 0) return false;
            String p1 = t.source;
            String p2 = orig.absolutePath();
            if (isDecodablePath(p1) || isDecodablePath(p2)) return true;
            if (AudioTranscoder.isTranscodableExtension(p1) || AudioTranscoder.isTranscodableExtension(p2)) return true;
            try {
                String sniff = sniffFileExt(orig);
                if (sniff != null) {
                    if (isDecodablePath("a" + sniff) || AudioTranscoder.isTranscodableExtension("a" + sniff)) return true;
                }
            } catch (Exception ignored) {}
            // 本地文件若扩展不在白名单但大小合理，仍视为可转码候选（如无扩展名但内容为音视频）
            return EXT_WHITELIST.matcher(p1).find() || EXT_WHITELIST.matcher(p2).find();
        }
        return false;
    }

    private static boolean isPlayableIndex(int idx) {
        if (idx < 0 || idx >= tracks.size) return false;
        return isPlayable(tracks.get(idx));
    }

    /** 本地声源句柄是否仍有效（未释放且在 voices 列表中），用于 idSeek 前的二次校验，避免对已释放句柄原生崩溃 */
    private static boolean isLocalVoiceValid() {
    ...
    public static String extensionFrom(String source) {
        ...
        if (dot >= 0) {
            ...
            // 若在白名单内则保留原始扩展名（供转码器识别真实容器），否则兜底 .ogg
            if (EXT_WHITELIST.matcher(e).find()) return e;
        }
        return ".ogg";
    }
    ...
    private static final class SoloudBridge {
        ...
        static void seek(int voiceId, float seconds) {
            try {
                if (voiceId < 0 || SEEK == null) return;
                // 二次校验：voice 必须仍 isPlaying，避免对已释放句柄 idSeek 导致 EXCEPTION_ACCESS_VIOLATION
                try { if (!arc.Core.audio.isPlaying(voiceId)) return; } catch (Throwable ignored) { return; }
                SEEK.invoke(null, voiceId, seconds);
            } catch (Throwable ignored) {
            }
        }
    }
