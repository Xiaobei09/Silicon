 ...
 * <p>
 * 策略（方式越多越好，覆盖桌面+Android）：
 * <ul>
 *   <li>1. 系统 ffmpeg / avconv（-vn -c:a libvorbis / pcm_s16le）—— 覆盖最广，音频/视频均可，桌面首选</li>
 *   <li>2. Android MediaExtractor + MediaCodec（反射调用，PCM 解码后写 wav）—— Android 无 ffmpeg 时的系统级回退</li>
 *   <li>3. javax.sound.sampled AudioSystem（SPI 扩展可含 flac/ogg/mp3 部分）—— 纯 Java 回退，桌面无 ffmpeg 时</li>
 *   <li>4. 直接 Soloud 原生（ogg/mp3/wav）—— 已可解码的免转码</li>
...
public class AudioTranscoder {
    ...
    public static Fi ensureDecodable(Fi src, String hash) {
        ...
        // 依次尝试：ffmpeg/avconv -> Android MediaExtractor -> Java AudioSystem
